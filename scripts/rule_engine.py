...
class RuleEngine:
    ...
    def apply_amount_formatting(self, text: str) -> Tuple[str, List[str]]:
        ...
        # 预处理：去掉已有的大写金额括号，避免替换后出现重复
        # 例如: ¥123,456元（人民币壹拾贰万叁仟肆佰伍拾陆元整）→ ¥123,456元
        text = re.sub(
            r'([¥￥]\s*[\d,]+(?:\.\d{1,2})?\s*元?)\s*[（(]\s*(?:人民币)?[壹贰叁肆伍陆柒捌玖拾佰仟万亿零]+元整\s*[）)]',
            r'\1',
            text
        )
    ...
    DATE_PATTERNS = [
        ...
        # 注意：[ \t]* 只匹配空格/制表符，避免\s*吞掉换行符导致行首黏连
        (re.compile(r'(\d{4})[ \t]*[-/年][ \t]*(\d{1,2})[ \t]*[-/月][ \t]*(\d{1,2})[ \t]*日?'),
         lambda m: f"{m.group(1)}年{int(m.group(2))}月{int(m.group(3))}日"),
        # 2026年2月3号 → 2026年2月3日
        (re.compile(r'(\d{4})[ \t]*年[ \t]*(\d{1,2})[ \t]*月[ \t]*(\d{1,2})[ \t]*号'),
         lambda m: f"{m.group(1)}年{int(m.group(2))}月{int(m.group(3))}日"),
    ]
